...
const PackOrder = ({ navigation }) => {
  ...
  const handlefetchstorepick = async (type = null, currentPage = 0) => {
    try {
      ...
      const response = await makeApiCall(
        API_URLS.fetch_stock_picking_pack,
        'POST',
        {
          params: {
            // ...filters,
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity style={styles.card} onPress={() => navigation.navigate(navigationString.PACKPREVIEW, { item })}>
