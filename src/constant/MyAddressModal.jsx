    ...
    const handleSubmit = async () => {
        try {
          ...
          const { isValid, errors } = validateAddress({
            name: newAddress.name,
            phone: newAddress.phone,
            houseNumber: newAddress.street1,
            state: selectedState?.label || "",
            city: selectedCity?.label || "",
            zip: newAddress.zip,
          });
          ...
          let params = {
            ...
            street2: newAddress.street2,
            zip: newAddress.zip,
            city: selectedCity ? { id: selectedCity.value, name: selectedCity.label } : null,
            state: selectedState ? { id: selectedState.value, name: selectedState.label } : null,
            country: selectedCountry
              ? {
                  id: selectedCountry.value,
                  name: selectedCountry.label,
          ...
          if (data?.id) {
            params = {
              ...
              phone_sanitized: data?.phone || newAddress.phone,
            };
          }
          ...
          if (response?.result?.message === "Success") {
            ...
            MessageShow.error("Error", response?.result?.errorMessage || "Save failed");
          }
        } catch (error) {
          console.log("Error saving address:", error);
          MessageShow.error("Error", "Unexpected error occurred");
        } finally {
          setisloding(false);
        }
      };
