...
Valid: ${formatDate(promotion.rule_date_from)} - ${formatDate(promotion.rule_date_to)}
...
${promotion.reward_type === 'discount' 
  ...
  : `Free Product: ${promotion.reward_product_id?.[0]?.name || 'Product reward'}`}
${promotion.description ? `Description: ${promotion.description}` : ''}
    `.trim()

    Alert.alert('Promotion Details', details, [
      { text: 'OK', style: 'default' }
    ])
  }

  const renderPromotionCard = (promotion) => {
    const isExpired = promotion.rule_date_to && new Date(promotion.rule_date_to) < new Date()
        ...
        <View style={styles.cardHeader}>
          <View style={styles.promotionTypeContainer}>
            <View style={[
              ...
              { backgroundColor: getPromotionTypeColor(promotion.reward_type, promotion.discount_type) }
            ]}>
              <Text style={styles.iconText}>
                {getPromotionIcon(promotion.reward_type, promotion.discount_type)}
              </Text>
            </View>
            <View style={styles.promotionInfo}>
              <Text style={[styles.promotionName, isExpired && styles.expiredText]} numberOfLines={2}>
                {promotion.name}
              </Text>
              <Text style={[styles.promotionCode, isExpired && styles.expiredText]}>
                {promotion.promo_code ? `Code: ${promotion.promo_code}` : 'No code required'}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.cardBody}>
          <View style={styles.offerContainer}>
            ...
            {promotion.rule_minimum_amount > 0 && (
              <Text style={[styles.minAmountText, isExpired && styles.expiredText]}>
          ...
          <View style={styles.validityContainer}>
            <Text style={[styles.validityLabel, isExpired && styles.expiredText]}>Valid until:</Text>
            <Text style={[
              styles.validityDate, 
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: COLORS.whiteColor}}>
      <Headercomp title={'Promotions'} left={true} onPress={() => navigation.goBack()}/>
    ...
    >
      {option.label}
        ...
        {isLoading ? (
          ...
          <ScrollView style={styles.promotionsContainer} showsVerticalScrollIndicator={false}>
