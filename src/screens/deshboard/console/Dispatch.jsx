...
const Dispatch = ({ navigation }) => {
  ...
  const handlefetchstorepick = async (type = null, currentPage = 0) => {
    try {
      ...
      const response = await makeApiCall(
        API_URLS.fetch_stock_picking_dispatch,
        'POST',
        {
          params: {
            type: type,
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity style={styles.card} onPress={() => navigation.navigate(navigationString.DISPATCHPREVIEW, { item })}>
        ...
        {item?.state==='done'&&<Text style={styles.label}>Status: <Text style={styles.status}>{item.state}</Text></Text>}
        {item.state==="confirmed"&&<Text style={styles.label}>Status: <Text style={styles.status}>Waiting</Text></Text>}
        {item.state=== "assigned"&&<Text style={styles.label}>Status: <Text style={styles.status}>Ready</Text></Text>}
        {item.state=== "cancel"&&<Text style={styles.label}>Status: <Text style={styles.status}>cancelled</Text></Text>}
